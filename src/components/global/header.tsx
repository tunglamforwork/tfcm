...
export const Header = ({ currentUser }: HeaderProps) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-nav-height z-50 p-4">
      <div className="relative h-full container flex items-center justify-between after:absolute after:left-0 after:bottom-0 after:h-[1px] after:w-full max-md:after:bg-border/50 md:after:bg-border-gradient">
        <div className="flex-1">
          <Link href="/" className="inline-flex items-center gap-3">
            ...
            <span className="font-semibold inline-block">TFCM</span>
          </Link>
        </div>
        <div
          className={cn(
          ...
          >
            {siteConfig.mainNav.map((navItem) => (
              <Link
                key={navItem.label}
                href={navItem.href}
                className="md:mx-2.5 text-2xl font-medium md:font-normal md:text-sm hover:text-primary transition-colors duration-300 px-10 md:px-0 py-5 md:py-0 border-b border-border/50 md:border-none"
